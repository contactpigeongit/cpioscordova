...
@objc(CPOrderData)
public class CPOrderData: NSObject, Codable {
    public var utmtid = ""
...
@objc(CPOrderItem)
public class CPOrderItem: NSObject, Codable {
    public var sku = ""
    public var name = ""
    public var qty = 0
    public var unitPrice: Decimal = 0.0
}

@objc(CPCartItem)
public class CPCartItem: NSObject, Codable {
    public var sku = ""
    public var name = ""
    public var qty = 0
    public var unitPrice: Decimal = 0.0
...
@objc(ScreenSize)
public class ScreenSize: NSObject {
...
public class CPConnectionService: NSObject{
    ...
    public func add2cart(pName:String, pSku:String, pQty:Int, pUnitPrice:Double, pImgURL:String, utmp:String) {
        ...
        let curCartItem = CPCartItem()
        curCartItem.sku = pSku
        curCartItem.name = pName
        curCartItem.qty = pQty
        curCartItem.unitPrice = jsonParameterDecimalFormatter(value: pUnitPrice)
        curCartItem.link = putmp
        curCartItem.image = pImgURL
    ...
    public func add2wlist(pName:String, pSku:String, pQty:Int, pUnitPrice:Double, pImgURL:String, utmp:String) {
        var putmp = utmp
        if (putmp == "") {
            putmp = "/"+CPMainParameters.shared.bundleID+"/ios/"+pName
        }
        ...
        let parameters = "action=event&cptoken=\(CPMainParameters.shared.cptoken)&utmipc=\(pSku)&utmipn=\(pName)&cf1=add2wlist&cf2=\(pQty)&cf3=\(pUnitPrice)&utmtid=&utmtto=&utmp=\(putmp)&utmdt=\(pName)&cuem=\(CPMainParameters.shared.cuem)&device_type=ios&device_id=\(CPMainParameters.shared.model)&device_osver=\(CPMainParameters.shared.systemVersion)&bundle_id=\(CPMainParameters.shared.bundleID)&cp_ver=\(CPMainParameters.shared.cp_ver)&cp_verClient=\(CPMainParameters.shared.cp_verClient)&utmsr=\(CPMainParameters.shared.utmsr)&language_code=\(CPMainParameters.shared.langStr)&ci=\(CPMainParameters.shared.ci)"
        printMsg(message: "add2cart parameters:\(parameters)")

        let url = URL(string: CPMainParameters.shared.cpSubmitDataEndpoint)!
        doPostRequest(url: url, parameters: parameters)
        printMsg(message: "add2wlist url:\(url)")
    }
    
    public func removefromwlist(pName:String, pSku:String, pQty:Int, pUnitPrice:Double, pImgURL:String, utmp:String) {
        var putmp = utmp
        if (putmp == "") {
            putmp = "/"+CPMainParameters.shared.bundleID+"/ios/"+pName
        }
        ...
        let parameters = "action=event&cptoken=\(CPMainParameters.shared.cptoken)&utmipc=\(pSku)&utmipn=\(pName)&cf1=removefromwlist&cf2=\(pQty)&cf3=\(pUnitPrice)&utmtid=&utmtto=&utmp=\(putmp)&utmdt=\(pName)&cuem=\(CPMainParameters.shared.cuem)&device_type=ios&device_id=\(CPMainParameters.shared.model)&device_osver=\(CPMainParameters.shared.systemVersion)&bundle_id=\(CPMainParameters.shared.bundleID)&cp_ver=\(CPMainParameters.shared.cp_ver)&cp_verClient=\(CPMainParameters.shared.cp_verClient)&utmsr=\(CPMainParameters.shared.utmsr)&language_code=\(CPMainParameters.shared.langStr)&ci=\(CPMainParameters.shared.ci)"
        printMsg(message: "removefromcart parameters:\(parameters)")

        let url = URL(string: CPMainParameters.shared.cpSubmitDataEndpoint)!
        printMsg(message: "removefromwlist url:\(url)")
    ...
    public func addOrderItem(sku:String, name:String, quant: Int, price:Double) {
        let newOrderItem = CPOrderItem()
        newOrderItem.sku = sku
        newOrderItem.name = name
        newOrderItem.qty = quant
        newOrderItem.unitPrice = jsonParameterDecimalFormatter(value: (ceil(price*100)/100))
    ...
    public func alterCart(curCartItem: CPCartItem, cartAction: String){
        ...
        if cartItemIndex > -1 {
            if cartAction == "add" {
                printMsg(message: "alertCart old Qty:\(String(describing: curCartScreenshot[cartItemIndex].qty))")
                curCartScreenshot[cartItemIndex].qty = curCartScreenshot[cartItemIndex].qty + curCartItem.qty
                printMsg(message: "alertCart new Qty:\(String(describing: curCartScreenshot[cartItemIndex].qty))")
    ...
    public func postCustomCart(curCartItems: [CPCartItem]){
        let jsonEncoder = JSONEncoder()
        let jsonData = try! jsonEncoder.encode(curCartItems)
        let json = String(data: jsonData, encoding: String.Encoding.utf8)
    ...
    func jsonParameterDecimalFormatter(value: Double) -> Decimal {
        let decimalNumber = Decimal.init(string: value.description)
